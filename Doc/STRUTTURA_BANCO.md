...
### 1.4 Regola funzionale di `Salva`
`Salva` non introduce una terza chiusura utente separata.

Nel flusso Banco reale:
- su una vendita nuova o su un documento ufficiale non fiscalizzato, `Salva` converge sulla stessa linea funzionale della `Cortesia`, ma senza stampa;
- il documento resta ufficiale, non fiscalizzato e aggiornabile sullo stesso riferimento legacy;
- se il documento non fiscalizzato viene riaperto dalla lista `Documenti`, l'operatore puo` modificare liberamente righe, quantita` e totale e richiudere con `Salva` sullo stesso `DocumentoGestionaleOid`;
- se invece viene riaperto un documento gia` fiscalizzato, la correzione resta controllata: si possono aggiornare gli articoli senza intaccare il totale gia` scontrinato;
- eventuali residui storici `Pubblicato Banco` vanno trattati solo come traccia legacy da riallineare, non come esito da reintrodurre nei nuovi flussi.
...
Applicazione esplicita al ramo `Articolo`:
- le immagini articolo usano `articoloimmagine` come legame DB e la cartella immagini configurata di `FM` come sorgente file reale;
- l'e-commerce non va dedotto dalle immagini o da URL remoti: il flag operativo reale e` `articolo.Online`, mentre la disponibilita`/stato collegato usa il lookup `articolo.Disponibilitaonline -> disponibilitaonline`;
- finche` `Online` non e` attivo, articolo e immagini restano nel perimetro locale legacy/FM; l'eventuale sincronizzazione verso Prestashop e` un passaggio successivo, non la sorgente operativa del dato.
...
- la ricerca anagrafica della scheda e` `padre-first`: se testo o barcode portano a una variante reale, il risultato operativo corretto e` aprire la scheda master/padre a cui quella variante appartiene;
- questa risalita al padre vale solo per `Articolo magazzino` e non deve contaminare i flussi che movimentano il pezzo reale;
- la relazione padre-varianti va comunque preservata, perche` la scheda padre dovra` mostrare in seguito varianti collegate, barcode e dettagli di famiglia.
...
- su documenti nuovi o non fiscalizzati salva senza stampa, convergendo operativamente sulla stessa linea della `Cortesia`;
- su documenti non fiscalizzati recuperati dalla lista permette anche modifiche che alterano il totale, sempre sullo stesso `DocumentoGestionaleOid`;
- se il documento era gia` fiscalizzato e viene aperto in correzione controllata, aggiorna lo stesso documento senza nuova stampa e senza generare un nuovo documento, ma con totale scontrinato protetto;
- se un documento non fiscalizzato con componente `Sospeso` viene chiuso in `Carta` / percorso che richiede scontrino, `Salva` deve annullare il precedente documento non fiscalizzato e rigenerarlo come nuovo `Scontrino` con data e numero correnti.
...
Questo vale per:
- documenti salvati senza stampa;
- `Cortesia` non fiscalizzata;
- eventuali documenti storici ancora letti come `Pubblicato Banco`.
...
- salvataggi non fiscalizzati = stessa famiglia visiva dei recuperabili non scontrinati
- eventuali residui storici `Pubblicato Banco` = verde più neutro e distinto, solo finche' ancora presenti nei dati storici
...
- quando l'operatore digita manualmente, il flusso corretto apre un lookup articoli dedicato con lista risultati e dettaglio laterale, senza auto-inserire subito la riga;
- solo una scansione barcode riconosciuta come input da pistola puo` consumare il token e inserire direttamente l'articolo nel documento;
- il dettaglio del lookup deve leggere solo dati legacy reali delle tabelle collegate davvero coinvolte nel catalogo articolo, senza campi inventati o persistenze parallele.
- la semantica corretta di questa famiglia di ricerca e` `variante-first`: `Banco`, `Carico merce`, `Scarico`, `Inventario`, `Reso`, `Omaggio` e flussi simili devono arrivare sempre al pezzo reale da movimentare, non fermarsi al padre anagrafico;
- il padre puo` essere usato come semplificazione visiva temporanea nel lookup, ma il risultato finale di un flusso operativo deve restare la variante reale da scaricare/caricare.
...
- la vecchia euristica basata sulle sole forme pagamento legacy non e` piu` da considerare regola viva;
- per la lista universale documenti di `FM`, la verifica reale del 2026-04-22 ha mostrato che la colonna `Scontrino` si allinea prima di tutto a `documento.Fatturato`:
  - `0 = No`
  - `1 = Si`
  - `4 = Si!`
- questo mapping e` coerente con il dump `FM`, che espone `Enums.EnumStatoScontrino.NonScontrinato = 0`, `ScontrinatoSi = 1` e `ScontrinatoSiForzatura = 4`;
- il bucket operativo dei non scontrinati copre quindi almeno i casi con `Fatturato = 0`, inclusi `Salva`, `Cortesia`, `Sospeso` e i casi recuperabili non fiscalizzati;
