...
---
## Regola architetturale UI/moduli

- La logica applicativa non deve dipendere dal framework grafico.
- I file .xaml definiscono solo layout, binding, stili e struttura visuale.
- I file .xaml.cs devono contenere solo wiring UI, eventi visuali e codice strettamente grafico.
- I ViewModel non devono dipendere da Window, UserControl, MessageBox, Dispatcher o altri tipi WPF-specifici.
- I modali/dialog sono consentiti, ma devono essere gestiti tramite interfacce, eventi o servizi astratti (es. IDialogService / IModalService).
- La business logic deve stare in servizi, domain e application layer.
- Ogni nuovo modulo deve essere progettato in modo da poter sostituire la UI WPF con un’altra UI (es. Avalonia) senza riscrivere la logica.
- Se un comportamento richiede una finestra modale, il ViewModel deve esprimere l’intento, non istanziare direttamente la finestra.
- Evitare riferimenti a System.Windows fuori dal layer UI.
...
1. `Doc/STRUTTURA_BANCO.md`
2. `Doc/DIARIO.md`
3. `Doc/DIARIO-2.md`
4. `Doc/architecture-ui.md`
...
- su una vendita nuova o su un documento ufficiale non fiscalizzato converge sulla stessa linea funzionale della `Cortesia`, ma senza stampa;
- su un documento già fiscalizzato aperto in correzione controllata aggiorna lo stesso documento senza nuova stampa, mantenendo protetto il totale già scontrinato.
...
- durante questa correzione il totale già scontrinato resta protetto e non deve essere alterato dalle modifiche riga;
...
- se è una `Cortesia` o comunque un documento non fiscalizzato, resta liberamente modificabile anche nel totale e si salva sullo stesso riferimento legacy;
...
- i componenti shared devono usare sempre prima le librerie custom del progetto gia` esistenti, invece di duplicare controlli o risorse dentro il singolo modulo;
- quando nasce una nuova libreria custom riusabile del progetto, va creata sotto la cartella `lib/` in root e non dentro l'host applicativo o in cartelle locali di modulo;
...
### 5. Regola librerie custom
Per componenti, controlli, behavior, risorse shared e primitive UI:
- usare sempre prima una libreria custom gia` esistente del progetto se copre il bisogno reale;
- non duplicare in locale componenti shared gia` presenti in `lib/`;
- se serve una nuova libreria custom riusabile, crearla direttamente sotto `lib/`;
- `Banco.UI.Wpf` deve restare host applicativo e contenitore delle view, non la posizione predefinita delle librerie shared.
...
- aggiornare `Doc/DIARIO-2.md` per tracciare la decisione o la migrazione reale chiusa.
